...
- `assets/logo-full.png` — logo officiel ELEV, encore affiché dans la sidebar des 5 pages pas encore harmonisées (Objectifs, Plan, Profil, Équipements, Paramètres) ainsi que sur `inspecteur.html`. `index.html`, `historique.html`, `activite.html` et `analyse.html` utilisent désormais la sidebar harmonisée (icône montagne + « ELEV » en texte, voir décisions section 14) — à étendre aux 5 pages restantes lors de leur refonte (voir question ouverte section 15).
