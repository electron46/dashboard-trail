...
- `assets/logo-full.png` — logo officiel ELEV, encore affiché dans la sidebar des 6 pages pas encore harmonisées (Analyse, Objectifs, Plan, Profil, Équipements, Paramètres) ainsi que sur `inspecteur.html`. `index.html`, `historique.html` et `activite.html` utilisent désormais la sidebar harmonisée (icône montagne + « ELEV » en texte, voir décisions section 14) — à étendre aux 6 pages restantes lors de leur refonte (voir question ouverte section 15).
...
- Sidebar harmonisée (icône montagne + texte « ELEV », séparateur nav produit/compte, bloc utilisateur en bas) étendue à `historique.html` et `activite.html` (2026-08-14) ; reste à faire sur les 6 pages pas encore refaites (Analyse, Objectifs, Plan, Profil, Équipements, Paramètres) — à harmoniser au fur et à mesure de leur refonte.
